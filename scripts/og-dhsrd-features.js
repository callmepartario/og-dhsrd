...
{ label: "Minor Health Potion Recipe", name: "minor-heath-potion-recipe", pages: ["Page 129"], errata: false, feature: "As a <a href='#define-downtime'-move>downtime move</a>, you can use a vial of blood to craft a <a href='#define-loot-minor-health-potion'>Minor Health Potion</a>." },
...
{ label: "Mythic Dust Recipe", name: "mythic-dust-recipe", pages: ["Page 131"], errata: false, feature: "As a <a href='#define-downtime'-move>downtime move</a>, you can use a handful of fine gold dust to craft <a href='#define-loot-mythic-dust'>Mythic Dust</a>." },
...
function ccChooseSubclass() {
	...
	// suggest traits
	let tArray = "";
	tArray += "<h4 class='og-tab'>" + classesList[ccUserClass].label + " Traits</h4>";
	...
	for (let i = 0; i < traitList.length; i++) {
		...
		tArray += "<h5 class='h6'>" + linkTrait(traitList[i].label) + "&nbsp;&nbsp;" + classesList[ccUserClass].traitarray[i] + "</h5>";
	...
	classItem += "<p>" + classesList[ccUserClass].items + "</p>"; // pop id
	document.getElementById("ccUserClassItem").innerHTML = classItem;
	// populate class domains
	let classDomains ="";
	classDomains += getCCnote("As <strong>" + linkClass(classesList[ccUserClass].label) + "</strong>, you begin with two <strong><a href='#define-domain-card'>domain cards</a></strong></strong> from the <strong>" + linkDomain(classesList[ccUserClass].domains[0]) + "</strong> and <strong>" + linkDomain(classesList[ccUserClass].domains[1]) + "</strong> domains.</p>");
...
function printClasses() {
	...
	for (let i = 0; i < classesList.length; i++) {
		...
		classRecord += "<li class='d-block'><strong>Class Items:</strong> " + classesList[i].items + "</li>";
		classRecord += "</ul></div>";
...
function printEquipment() {
	...
	for (i = 0; i < consumableList.length; i++) {
		...
		consumableTable += "<td><div class='og-item' id='define-loot-" + consumableList[i].name + "'><strong><em>" + consumableList[i].label + ":</em></strong> " + consumableList[i].feature + " </div></td>";
		consumableTable += "</tr>";
	}
...
function printAdversaries() {
	...
	for (let a = 0; a < adversaryList.length; a++) {
		...
		iAdv += "<li class='d-block'><strong>ATK:</strong> " + adversaryList[a].attackbonus + "<span class='og-divider'><strong>" + adversaryList[a].attacklabel + ":</strong> <a href='#define-range'>" + adversaryList[a].attackrange + "</a></span><span class='og-divider'>" + adversaryList[a].attackdamage + " <a href='#define-damage-type'>" + adversaryList[a].attackdamagetype + "</a></span></li>";
